...
pub const SEED_MAX_BYTES: usize = ELEM_MAX_BYTES;
pub const PUBLIC_KEY_MAX_LEN: usize = 1 + (2 * ELEM_MAX_BYTES);

pub const ED25519_PKCS8_TEMPLATE: Template = Template {
...
pub const EC_PUBLIC_KEY_P256_PKCS8_V1_TEMPLATE: Template = Template {
...
pub const EC_PUBLIC_KEY_P384_PKCS8_V1_TEMPLATE: Template = Template {
...
pub fn wrap_sk(template: &Template, private_key: &[u8]) -> Document {
    let mut result = Document {
        bytes: [0; PKCS8_DOCUMENT_MAX_LEN],
        len: template.bytes.len() + private_key.len(),
    };
    wrap_sk_(template, private_key, &mut result.bytes[..result.len]);
    result
}
...
/// Formats a private key "prefix||private_key||middle" where
/// `template` is "prefix||middle" split at position `private_key_index`.
fn wrap_sk_(template: &Template, private_key: &[u8], bytes: &mut [u8]) {
    let (before_private_key, after_private_key) =
        template.bytes.split_at(template.private_key_index);
    let private_key_end_index = template.private_key_index + private_key.len();
    bytes[..template.private_key_index].copy_from_slice(before_private_key);
