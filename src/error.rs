...
///
/// `Result<T, pkcs8::Unspecified>` is mostly equivalent to
/// `Result<T, ()>`. However, `pkcs8::Unspecified` implements
/// [`std::error::Error`] and users of *ring* can implement
/// `From<pkcs8::Unspecified>` to map this to their own error types, as
...
/// }
///
/// impl From<cellar_pkcs8::Unspecified> for Error {
///     fn from(_: cellar_pkcs8::Unspecified) -> Self { Error::CryptoError }
/// }
///
...
///     let mut bytes = [0; 8];
///
///     // The `From<pkcs8::Unspecified>` implementation above makes this
///     // equivalent to
///     // `rng.fill_bytes(&mut bytes).map_err(|_| Error::CryptoError)?`.
...
impl KeyRejected {
    ...
    pub fn public_key_is_missing() -> Self {
    ...
    pub fn wrong_algorithm() -> Self {
        KeyRejected("WrongAlgorithm")
    }

    #[cfg(feature = "alloc")]
    pub fn private_modulus_len_not_multiple_of_512_bits() -> Self {
